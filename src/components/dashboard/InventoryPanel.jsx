...
const ITEMS = [
  ...
  { key: 'paquetes_tapas',     label: 'Paquetes de Tapas',     icon: '🔵', unit: 'paquetes', equiv: '1 paquete = tapas'        },
  { key: 'paquetes_etiquetas', label: 'Paquetes de Etiquetas', icon: '🏷️', unit: 'paquetes', equiv: '1 paquete = etiquetas'    },
];
...
export default function InventoryPanel({ almacen, onRefresh }) {
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      await updateAlmacen(almacen.id, form);
      const cambios = ITEMS.map((item) =>
        `${item.label}: ${almacen[item.key]} → ${form[item.key]}`
      ).join(' | ');
      await registrarLog(user.id, 'modificacion_inventario', `Almacen actualizado: ${cambios}`);
    ...
    <div className="cv-dash-panel">
      <div className="cv-dash-panel__header">
        ...
        <div className="cv-dash-panel__actions">
          {saveMsg && <span className="cv-dash-panel__msg">{saveMsg}</span>}
          {isAdmin && !editMode && (
            <motion.button
              className="cv-dash-btn cv-dash-btn--edit"
              onClick={startEdit}
          ...
          {editMode && (
            <>
              ...
              <motion.button
                className="cv-dash-btn cv-dash-btn--cancel"
                onClick={cancelEdit}
                whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.97 }}
              >
                ✕ Cancelar
